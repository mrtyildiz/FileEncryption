...
from FileEncryptRSApkcs11 import rsa_encrypt_file, rsa_decrypt_file
import os
app = FastAPI()
...
class Encryptpkcs11(BaseModel):
    ID: int
    PIN: str
    FileName: str
    PublicKeyName: str

class Decryptpkcs11(BaseModel):
    ID: int
    PIN: str
    FileEncName: str
    PrivateKeyName: str
...
async def upload_file(file: UploadFile = File(...)):
    ...
    file_destination = f"{file_path}/{file.filename}"
...
@app.post("/download")
def download_file(filename: str = Form(...)):
    path = file_path = os.environ.get("File_Path")
...
def RSA_File_Enc_Value(data: FileEncryptRSA):
    Slot_ID = data.ID
    Slot_PIN = data.PIN
...
def RSA_File_Dec_Value(data: FileDecryptRSA):
    Slot_ID = data.ID
    Slot_PIN = data.PIN
...
@app.post("/RSAFileEnc")
def RSA_Enc(data: Encryptpkcs11):
    Slot_ID = data.ID
    Slot_PIN = data.PIN
    File_Name = data.FileName
    PubName = data.PublicKeyName
    result = rsa_encrypt_file(Slot_ID, File_Name, PubName)
    return result

@app.post("/RSAFileDec")
def RSA_Dec(data: Decryptpkcs11):
    Slot_ID = data.ID
    Slot_PIN = data.PIN
    File_Name = data.FileEncName
    Priv_Name = data.PrivateKeyName
    result = rsa_decrypt_file(Slot_ID, Slot_PIN, File_Name, Priv_Name)
    return result
