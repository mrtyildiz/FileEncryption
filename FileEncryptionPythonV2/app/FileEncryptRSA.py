...
def encrypt_file_rsa(Slot_ID, file, public_key_label, pin):
    try:
        file_root = os.environ.get('File_Path')
        file_path = str(file_root)+"/"+str(file)
        pkcs11_lib = os.environ.get('HSM_SO_FILE')
        ...
        result = "File encrypted error"
        return result

def decrypt_file_rsa(Slot_ID, file, private_key_label, pin):
    try:
        file_root = os.environ.get('File_Path')
        file_path = str(file_root)+"/"+str(file)
        pkcs11_lib = os.environ.get('HSM_SO_FILE')
